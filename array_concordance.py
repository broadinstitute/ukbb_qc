...
def prepare_array_and_exome_mt(data_source, freeze, array_mt, exome_mt, call_rate_cutoff, af_cutoff):
    ...
    sample_map = hl.read_table(array_sample_map_ht('broad', freeze))
    ...
    logger.info(f'Total number of IDs in the sample map that are also in the array data: {exome_array_count}...')

    array_mt = hl.variant_qc(array_mt)
    array_mt = array_mt.checkpoint(
        get_mt_checkpoint_path(data_source, freeze, name=f'array_mt_variant_qc'), overwrite=True)
    array_mt = array_mt.filter_rows((array_mt.variant_qc.call_rate > call_rate_cutoff) &
    ...
    exome_mt = exome_mt.filter_rows(hl.is_defined(array_mt.index_rows(exome_mt.row_key)))

    exome_mt = hl.variant_qc(exome_mt)
    exome_mt = exome_mt.checkpoint(
        get_mt_checkpoint_path(data_source, freeze, name=f'exome_mt_variant_qc'), overwrite=True)
    exome_mt = exome_mt.filter_rows((exome_mt.variant_qc.call_rate > call_rate_cutoff) &
...
def get_array_exome_concordance(array_mt, exome_mt):
    ...
    variants = variants.annotate(
        num_gt_con=variants.concordance[2][2] + variants.concordance[3][3] + variants.concordance[4][4],
        num_gt=hl.sum(variants.concordance[2][2:]) + hl.sum(variants.concordance[3][2:]) + hl.sum(variants.concordance[4][2:]),
        num_gt_con_non_ref=variants.concordance[3][3] + variants.concordance[4][4],
        num_gt_non_ref=hl.sum(variants.concordance[2][3:]) + hl.sum(variants.concordance[3][2:]) + hl.sum(variants.concordance[4][2:]))
    variants = variants.annotate(prop_gt_con=(variants.num_gt_con / variants.num_gt),
    ...
    samples = samples.annotate(
        ...
        num_gt_non_ref=hl.sum(samples.concordance[2][3:]) + hl.sum(samples.concordance[3][2:]) + hl.sum(samples.concordance[4][2:]))
...
def main(args):
    ...
    if args.liftover_arrays:
        ...
        array_mt = lift_data(array_mt, gnomad=False, data_type=None,
                             path=array_mt_path(liftover=True, checkpoint=True), rg=target,
                             overwrite=overwrite)

        logger.info('Checking SNPs for reference mismatches')
        array_mt = annotate_snp_mismatch(array_mt, data_type=None, rg=target)
        array_mt = array_mt.checkpoint(array_mt_path(liftover=True), overwrite=overwrite)
        array_variants, array_samples = array_mt.count()
    ...
    if args.array_concordance:
        ...
        if args.interval_qc_filter:
            exome_mt = interval_qc_filter(data_source, freeze, exome_mt, args.pct_samples_20x)

        array_mt, exome_mt = prepare_array_and_exome_mt(data_source,freeze,array_mt, exome_mt, call_rate_cutoff, af_cutoff)

        exome_mt = exome_mt.checkpoint(
            get_mt_checkpoint_path(data_source, freeze, name=f'exome_subset_concordance_callrate_{call_rate_cutoff}_af_{af_cutoff}'),
            overwrite=overwrite)
        array_mt = array_mt.checkpoint(
            get_mt_checkpoint_path(data_source, freeze, name=f'array_subset_concordance_callrate_{call_rate_cutoff}_af_{af_cutoff}'),
            overwrite=overwrite)

        samples, variants = get_array_exome_concordance(array_mt, exome_mt)

        variants = variants.annotate_globals(callrate_cutoff=call_rate_cutoff,
                                             af_cutoff=af_cutoff)
        samples = samples.annotate_globals(callrate_cutoff=call_rate_cutoff,
                                             af_cutoff=af_cutoff)

        variants.write(array_variant_concordance_path(data_source, freeze), overwrite=overwrite)
        samples.write(array_sample_concordance_path(data_source, freeze), overwrite=overwrite)
    ...
    if args.make_variant_qc_ht:
        ...
        exome_mt = hl.read_matrix_table(get_mt_checkpoint_path(
            data_source,
            freeze,
        ...
        variants = variants.filter((variants.prop_gt_con_non_ref > args.concordance_cutoff) & (variants.AF > args.variant_qc_af_cutoff))
        variants = variants.repartition(1000)
        variants.write(
            f'{variant_qc_prefix(data_source, freeze)}/array_variant_concordance_callrate_{callrate_cutoff}_non_ref_con_{args.concordance_cutoff}_AF_{args.variant_qc_af_cutoff}.ht',
            overwrite=overwrite)
...
if __name__ == '__main__':
    ...
    array_import.add_argument('-l', '--liftover_arrays', help='Liftover array Matrix Table to GRCh38',
                              action='store_true')
    ...
    concordance.add_argument('--call_rate_cutoff', help='Call rate cutoff.', type=float,
                             default=0.95)
    concordance.add_argument('--af_cutoff', help='Allele frequency cutoff (also used for variant QC HT creation).',
                             type=float,
                             default=0.0001)
    ...
    parser.add_argument('--pct_samples_20x',
                        ...
                        default=0.85,
                        type=float)
    concordance.add_argument('--make_variant_qc_ht', help='Make HT for concordance HT for variant QC',
                             action='store_true')
    concordance.add_argument('--concordance_cutoff', help='Array exome concordance cutoff for variant QC HT.',
                             type=float,
                             default=0.9)
    concordance.add_argument('--variant_qc_af_cutoff',
