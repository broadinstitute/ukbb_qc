...
def create_regeneron_ht(data_source, freeze):
    ...
    nf_ht = nf_mt.annotate_rows(
        singleton=hl.agg.sum(nf_mt.GT.n_alt_alleles()) == 1
    ).rows()
    gl_ht = pre_process_mt(
        f"gs://broad-ukbb/regeneron.freeze_{freeze}/data/regeneron.freeze_{freeze}.gl.mt"
    )
    ht = nf_ht.annotate(
        pass_regeneron=hl.is_defined(gl_ht.index(nf_ht.key)), score=nf_ht.qual
    )

    return ht.checkpoint(
        f"{variant_qc_prefix(data_source, freeze)}/assessment/regeneron.freeze_{freeze}.nf.gl_annotated.ht",
...
def create_binned_concordance(
    ...
    if (
        ...
        ht = hl.read_table(
            f"{variant_qc_prefix(data_source, freeze)}/assessment/joint_nist_{data_source}.{truth_sample}.{args.run_hash}.ht"
        )
        ...
        if metric == "vqsr":
            ...
            metric_ht = hl.read_table(
                "gs://gnomad-public/release/2.1.1/liftover_grch38/ht/exomes/gnomad.exomes.r2.1.1.sites.liftover_grch38.ht"
            )
...
def main(args):
    ...
    if args.import_truth_vcfs:
        for sample_id, file_handle in truth_samples.items():
            mt = hl.import_vcf(
                f"gs://broad-ukbb/resources/nist/{file_handle}",
                force_bgz=True,
                reference_genome="GRCh38",
                header_file="gs://broad-ukbb/resources/nist/nist_header_for_import.txt",
            )
            mt = mt.checkpoint(
                f"gs://broad-ukbb/resources/nist/{sample_id}.mt",
                overwrite=args.overwrite,
            )
            mt.summarize()
            ...
            ht.summarize()
            ht.write(
                f"gs://broad-ukbb/resources/nist/{sample_id}.ht",
                overwrite=args.overwrite,
            )
    ...
    if args.match_raw_sites:
        for sample_id in truth_samples.keys():
            ...
            if args.run_hash == "vqsr":
                ht = hl.read_table(
                    score_ranking_path(data_source, freeze, args.run_hash)
                )
                ...
                if not hl.hadoop_exists(
                    f"{variant_qc_prefix(data_source, freeze)}/assessment/regeneron.freeze_{freeze}.nf.gl_annotated.ht/_SUCCESS"
                ):
                ...
                ht = ht.annotate(
                    pass_broad=(ht.rf_probability.get("TP") > args.tp_threshold)
                )

            # Match sites on locus and allele
            joint_ht = (
                nist_ht.join(ht, how="inner")
                .naive_coalesce(500)
                .checkpoint(
                    f"{variant_qc_prefix(data_source, freeze)}/assessment/joint_nist_{data_source}.{sample_id}.{args.run_hash}.ht",
                    overwrite=args.overwrite,
                )
            )
            filter_summary = joint_ht.group_by(
                joint_ht.pass_nist, joint_ht[f"pass_{data_source}"]
            ).aggregate(n=hl.agg.count())
            filter_summary.show()

            joint_ht = joint_ht.annotate(
                is_snp=hl.is_snp(joint_ht.alleles[0], joint_ht.alleles[1])
            )
            filter_summary = joint_ht.group_by(
                joint_ht.is_snp,
                joint_ht.was_split_1,
                joint_ht.pass_nist,
                joint_ht[f"pass_{data_source}"],
            ).aggregate(n=hl.agg.count())
    ...
    if args.filter_difficult_regions:
        for sample_id in truth_samples.keys():
            ...
            joint_ht = joint_ht.filter(
                hl.is_defined(joint_ht.info.difficultregion), keep=False
            )
    ...
    if args.compute_binned_concordance:
        for sample_id in truth_samples.keys():
            create_binned_concordance(
                ...
                args.n_bins,
                overwrite=args.overwrite,
                truth_sample=sample_id,
                filter_difficult_regions=args.filter_difficult_regions,
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--import_truth_vcfs", help="Import NIST VCFs", action="store_true"
    )
    parser.add_argument(
        "--match_raw_sites",
        help="Compare filter status in Broad and NIST VCFs",
        action="store_true",
    )
    parser.add_argument(
        "--compute_binned_concordance",
        help="Compute site concordance between Broad and NIST calls with binned ranking annotations",
        action="store_true",
    )
    parser.add_argument(
        "--filter_difficult_regions",
        help='Remove NIST variants that occur in at least one "difficult region" track',
        action="store_true",
    )
