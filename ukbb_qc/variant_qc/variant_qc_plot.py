...
def get_binned_concordance_pd(data_source: str, freeze, truth_samples: List[str], models: Union[Dict[str, str], List[str]]) -> pd.DataFrame:
    """
    ...
    def get_binned_concordance_ht(data_source: str, freeze, truth_samples: List[str], models: Dict[str, str]) -> hl.Table:
        """
        ...
        for truth_sample in truth_samples:
            for model_id, model_name in models.items():
                ht = hl.read_table(binned_concordance_path(truth_sample, model_id, data_source, freeze))
    ...
    df = df.groupby(['bin_id', 'truth_sample', 'model', 'snv']).apply(compute_cumul_metrics)
    return df.fillna(-1).groupby(['bin_id', 'truth_sample', 'model', 'snv'])
...
def get_binned_concordance_chr20_pd(
    ...
    # print(df.head())
    df = df.groupby(["bin_name", "truth_sample", "model", "snv"]).apply(
        compute_cumul_metrics
    )
    mins = df.groupby(["snv"]).min()
    return df.fillna(-1).groupby(["bin_name", "truth_sample", "model", "snv"]), mins
...
def get_binned_models_pd(
    ...
    hts = [
        aggregate_contig(
            hl.read_table(
                score_quantile_bin_path(model_id, data_source, freeze, aggregated=True)
            ),
            contigs,
...
def plot_score_distributions(
    ...
    for model_name in model_order:
        ...
        if model_id in ["vqsr", "AS_TS_vqsr", "cnn", "rf_2.0.2", "rf_2.0.2_beta"]:
            ht = hl.read_table(
                score_quantile_bin_path(model_id, data_source, freeze, aggregated=False)
            )
        else:
            ht = hl.read_table(
                rf_path(data_source, freeze, "rf_result", run_hash=model_id)
            )
            ht = ht.annotate(score=ht.rf_probability["TP"])
        ...
        if cut_is_bin:
            cut_value = binned_ht.aggregate(
                hl.agg.filter(
                    (binned_ht.bin == cut)
                    & (binned_ht.bin_id == f"{rank_prefix}bin"),
        ...
        agg_values = ht.aggregate(
            hl.struct(
                score_hist=[
                    ...
                    hl.agg.filter(
                        ht.ac > 0, hl.agg.hist(ht.score, min_score, max_score, 100)
                    ),
                ],
...
def plot_metric(
    ...
    DataFrame needs to have a `bin_id` column, a `bin` column and a `model` column (contains the model name and needs to be added to binned table(s))
    ...
    if plot_all:
        ...
        for adj in adj_strats:
            ...
            plot_df = prepare_pd(
                df.loc[df.bin_id == f"{rank_prefix}{adj}bin"], cols, colors, size_prop
            )
            if len(plot_df) > 0:
                children.append(
                    get_row(
        ...
        for adj in adj_strats:
            for biallelic_rank in ["", "biallelic_"]:
                ...
                plot_df = prepare_pd(
                    df.loc[
                        df.bi_allelic
                        & (df.bin_id == f"{rank_prefix}{adj}{biallelic_rank}bin")
                    ],
                    cols,
                    colors,
                    size_prop,
        ...
        for adj in adj_strats:
            for singleton_rank in ["", "singleton_"]:
                ...
                plot_df = prepare_pd(
                    df.loc[
                        df.singleton
                        & (df.bin_id == f"{rank_prefix}{adj}{singleton_rank}bin")
                    ],
                    cols,
                    colors,
                    size_prop,
            ...
            for bisingleton_rank in ["", "biallelic_singleton_"]:
                ...
                plot_df = prepare_pd(
                    df.loc[
                        df.bi_allelic
                        & df.singleton
                        & (df.bin_id == f"{rank_prefix}{adj}{bisingleton_rank}bin")
                    ],
                    cols,
                    colors,
                    size_prop,
...
def plot_concordance_pr(
    ...
    tabs = []
    for rank in ['truth_sample_bin', 'global_bin','truth_sample_interval_bin', 'global_interval_bin']:
...
def plot_feature_importance(data_source, freeze, models: Dict[str, str], model_order: Optional[List[str]] = None):
    """
    Barplot of feature importances with one tab per model

    :param models: Dictionary defining a label for each model (key that is RF run hash, 'vqsr', or 'AS_TS_vqsr')
    :param model_order: The order of models in the tabs
    :return:
    """
    feature_map = {'AS_QD': 'Quality by depth', 'InbreedingCoeff': 'Inbreeding coefficient',
                   'allele_type': 'Allele type', 'was_mixed': 'Mixed site',
                   'AS_ReadPosRankSum': 'Read position ranksum',
                   'variant_type': 'Variant type', 'AS_MQRankSum': 'Mapping quality ranksum',
                   'n_alt_alleles': 'Number of alleles', 'AS_SOR': 'Strand bias odds ratio',
                   'AS_pab_max': 'Max probability of allele balance', 'has_star': 'Spanning deletion'}

    models = {v: k for k, v in models.items()}
    if model_order is None:
        model_order = models.values()

    tabs = []
    for label in model_order:
        model = models[label]
        if model in ['vqsr', 'AS_TS_vqsr']:
            continue
        rf_ht_features_importance = hl.read_table(
            rf_path(data_source, freeze, "rf_result", run_hash=model)
        ).features_importance.collect()[0]

        data = [(feature_map[k], v) for k, v in rf_ht_features_importance.items()]
        data.sort(key=operator.itemgetter(1))
        bars = hv.Bars(data, hv.Dimension('Feature'), 'Importance')
        bars.opts(invert_axes=True, height=700, width=1000, fontsize={'labels': 16, 'xticks': 12, 'yticks': 12})
        p = hv.render(bars, backend='bokeh')
