...
def create_rf_rank(data_source: str, freeze: int, run_hash: str) -> None:
    ...
    logger.info(f"Annotating RF file for {data_source}.freeze_{freeze} RF run {run_hash} with quantile bins")
    ht = ht.annotate(
        score=1-ht.rf_probability['TP']
    )
    bin_ht = default_create_binned_ht(ht, n_bins, add_substrat={"interval": ht.interval_qc_pass})
    ...
    grouped_binned_ht = compute_grouped_binned_ht(
        binned_ht,
        checkpoint_path=f"gs://broad-ukbb-jgoodric/broad_5_vqsr_binned.ht",
    )
...
def filter_clinvar_path():
    ...
    no_star_assertions = hl.literal(
        {"no_assertion_provided", "no_assertion_criteria_provided", "no_interpretation_for_the_single_variant"})
    clinvar_ht = clinvar_ht.filter(hl.set(clinvar_ht.info.CLNREVSTAT).intersection(no_star_assertions).length() > 0,
                                   keep=False)
    logger.info(f"Found {clinvar_ht.count()} variants after removing variants without assertions")
    clinvar_ht = clinvar_ht.filter(
        clinvar_ht.info.CLNSIG.map(lambda x: x.lower()).map(lambda x: x.contains('pathogenic')).any(lambda x: x),
        keep=True)
...
def create_binned_data(ht: hl.Table, data: str, data_source: str, freeze: int, n_bins: int) -> hl.Table:
    ...
    count_expr = {x: hl.agg.filter(hl.is_defined(ht[x]), hl.agg.counter(hl.cond(hl.is_snp(ht.alleles[0], ht.alleles[1]), 'snv', 'indel'))) for x in ht.row if x.endswith('bin')}
    ...
    if not hl.utils.hadoop_exists('gs://broad-ukbb/resources/clinvar_20190609.pathogenic.ht'):
        ...
        clinvar_path_ht.write('gs://broad-ukbb/resources/clinvar_20190609.pathogenic.ht')

    clinvar_path_ht = filter_clinvar_path()
    ht = ht.annotate(clinvar_path=hl.is_defined(clinvar_path_ht[ht.key]))
    trio_stats_ht = hl.read_table(var_annotations_ht_path(data_source, freeze, "trio_stats"))
