...
def write_truth_concordance(
    ...
    sample_mapping = {"NA12878": "Coriell_NA12878_NA12878"}

    truth_mt = hl.read_matrix_table(
        "gs://broad-ukbb/broad.freeze_4/data/NovaSeq_Coriell_Freeze.NF.truth_mt"
    )
    ...
    truth_mt = truth_mt.repartition(
        1000 if data_type == "genomes" else 100, shuffle=False
    )

    broad_ht = hl.read_table(
        "gs://broad-ukbb/broad.freeze_4/temp/joint_broad_regeneron.NA12878.ht"
    )
...
def create_binned_concordance(
    ...
    if (
        ...
        ht = hl.read_table(
            f"gs://broad-ukbb/{data_source}.freeze_{freeze}/temp/joint_broad_regeneron.NA12878.ht"
        )
        ...
        # )
        logger.info(
            f"Found {ht.count()} variants out of {count1} after removing LCR regions"
        )
        ...
        ht = ht.annotate(
            ...
            & ~hl.is_defined(
                ht.pass_reg
            ),  # pass_reg is missing for samples that do not pass in Regeneron
...
def main(args):
    ...
    if args.import_vcfs:
        ...
        grch37_contigs = [
            x for x in rg.contigs if not x.startswith("GL") and not x.startswith("M")
        ]
        contig_dict = dict(zip(grch37_contigs, ["chr" + x for x in grch37_contigs]))

        gl_mt = hl.import_vcf(
            "gs://broad-ukbb/broad.freeze_4/data/vcfs/NovaSeq_Coriell_Freeze.GL.vcf.gz",
            force_bgz=True,
            reference_genome="GRCh38",
            array_elements_required=False,
            contig_recoding=contig_dict,
        )
        gl_mt = gl_mt.checkpoint(
            "gs://broad-ukbb/broad.freeze_4/data/NovaSeq_Coriell_Freeze.GL.mt",
            overwrite=args.overwrite,
        )
        gl_mt.describe()
        gl_mt.summarize(entries=False)
        nf_mt = hl.import_vcf(
            "gs://broad-ukbb/broad.freeze_4/data/vcfs/NovaSeq_Coriell_Freeze.NF.vcf.gz",
            force_bgz=True,
            reference_genome="GRCh38",
            array_elements_required=False,
            contig_recoding=contig_dict,
        )
        nf_mt = nf_mt.checkpoint(
            "gs://broad-ukbb/broad.freeze_4/data/NovaSeq_Coriell_Freeze.NF.mt",
            overwrite=args.overwrite,
        )
        nf_mt.describe()
    ...
    if args.match_raw_sites:
        ...
        broad_ht = broad_ht.annotate(
            pass_broad=(broad_ht.rf_probability.get("TP") > 0.03)
        )
        nf_mt = hl.read_matrix_table(
            "gs://broad-ukbb/broad.freeze_4/data/NovaSeq_Coriell_Freeze.NF.mt"
        )
        gl_ht = hl.read_matrix_table(
            "gs://broad-ukbb/broad.freeze_4/data/NovaSeq_Coriell_Freeze.GL.mt"
        ).rows()
        gl_ht = gl_ht.annotate(pass_reg=True)

        regeneron_mt = nf_mt.filter_cols(nf_mt.s == "Coriell_NA12878_NA12878")
        regeneron_ht = regeneron_mt.filter_rows(
            hl.agg.sum(regeneron_mt.GT.n_alt_alleles()) > 0, keep=True
        ).rows()
        logger.info(
            f"Found {regeneron_ht.count()} variants in NA12878 out of {regeneron_mt.count_rows()} in callset"
        )
        regeneron_ht = regeneron_ht.join(gl_ht, how="left")

        joint_ht = regeneron_ht.join(broad_ht, how="inner").checkpoint(
            f"gs://broad-ukbb/{data_source}.freeze_{freeze}/temp/joint_broad_regeneron.NA12878.ht",
            overwrite=args.overwrite,
        )
        filter_summary = joint_ht.group_by(
            joint_ht.pass_broad, joint_ht.pass_reg
        ).aggregate(n=hl.agg.count())
        filter_summary.show()

    if args.compute_binned_concordance:
        create_binned_concordance(
            data_source, freeze, args.run_hash, args.n_bins, overwrite=args.overwrite
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--import_vcfs", help="Import NA12878 VCFs", action="store_true"
    )
    parser.add_argument(
        "--match_raw_sites",
        help="Compare filter status in Broad and Regeneron NA12878 VCFs",
        action="store_true",
    )
    parser.add_argument(
        "--compute_binned_concordance",
        help="Compute site concordance between Broad and Regeneron NA12878 calls with binned ranking annotations",
        action="store_true",
    )
