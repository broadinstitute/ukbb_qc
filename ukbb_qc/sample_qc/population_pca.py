...
def project_on_gnomad_pop_pcs(mt: hl.MatrixTable) -> hl.Table:
    ...
    gnomad_meta_exomes_ht = prep_meta(
        hl.read_table(metadata_exomes_ht_path(version=CURRENT_EXOME_META))
    )
    ...
    scores_ht = scores_ht.select(
        "pop_for_rf", **{f"PC{i + 1}": scores_ht.scores[i] for i in range(10)}
    )
...
def assign_cluster_pcs(
    ...
    logger.info(
        "Assigning clusters to {} exome samples in MT...".format(len(pc_scores))
    )
...
def load_ukbb_array_pcs(data_source, freeze, num_pcs=16):
    ...
    array_pcs = array_pcs.annotate(
        scores=hl.array([array_pcs[f"f{i+26}"] for i in range(0, num_pcs)])
    )
...
def main(args):
    ...
    if args.assign_clusters_array_pcs:
        ...
        pops_ht.annotate_globals(hdbscan_min_cluster_size=args.hdbscan_min_cluster_size)
        pops_ht.write(
            ancestry_cluster_array_ht_path(data_source, freeze), args.overwrite
        )
    ...
    if args.assign_clusters_joint_scratch_array_pcs:
        ...
        array_pc_ht = array_pc_ht.annotate(
            exome_scores=scores_ht[array_pc_ht.key].scores
        )
        ...
        array_pc_ht = array_pc_ht.select(
            ...
            **{
                f"PC{i + n_pcs + 1}": array_pc_ht.scores[i]
                for i in range(args.n_array_pcs)
            },
        )
        array_pc_ht = array_pc_ht.annotate(
            scores=hl.array(
                [array_pc_ht[f"PC{i + 1}"] for i in range(n_pcs + args.n_array_pcs)]
            )
        )
        ...
        pops_ht.annotate_globals(hdbscan_min_cluster_size=args.hdbscan_min_cluster_size)
        pops_ht.write(
            ancestry_cluster_joint_scratch_array_ht_path(data_source, freeze),
            args.overwrite,
        )
