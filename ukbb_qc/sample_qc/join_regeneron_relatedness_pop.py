...
def main(args):
    ...
    if args.join_relatedness:
        ...
        first_degree_threshold = [
            float(i) for i in args.first_degree_kin_threshold.split(",")
        ]
        ...
        regeneron_rel_SD_ht = hl.import_table(
            get_regeneron_relatedness_path(freeze, relationship="2nd-degree"),
            impute=True,
            delimiter=" ",
        )
        regeneron_rel_FS_ht = hl.import_table(
            get_regeneron_relatedness_path(freeze, relationship="full-sibling"),
            impute=True,
            delimiter=" ",
        )
        regeneron_rel_PC_ht = hl.import_table(
            get_regeneron_relatedness_path(freeze, relationship="parent-child"),
            impute=True,
            delimiter=" ",
        )
        regeneron_rel_ht = regeneron_rel_SD_ht.union(regeneron_rel_FS_ht).union(
            regeneron_rel_PC_ht
        )
        ...
        relatedness_ht = relatedness_ht.annotate(
            samples=hl.delimit(hl.sorted([relatedness_ht.i.s, relatedness_ht.j.s]))
        )
        regeneron_rel_ht = regeneron_rel_ht.annotate(
            samples=hl.delimit(
                hl.sorted([regeneron_rel_ht.IID1, regeneron_rel_ht.IID2])
            )
        )
        ...
        regeneron_rel_ht = regeneron_rel_ht.annotate(
            Regeneron_classification=(
                hl.switch(regeneron_rel_ht.MOST_LIKELY_REL)
        ...
        relatedness_joined_ht = relatedness_ht.key_by("samples").join(
            regeneron_rel_ht.key_by("samples"), "outer"
        )
        relatedness_joined_ht = relatedness_joined_ht.annotate(
            Regeneron_classification=hl.or_else(
                relatedness_joined_ht.Regeneron_classification, "None"
            ),
            Broad_classification=hl.or_else(
                relatedness_joined_ht.Broad_classification, "None"
            ),
        )

        relatedness_joined_ht.write(
            get_regeneron_broad_relatedness_path(data_source, freeze),
    ...
    if args.filter_ukbb_ancestry:
        ...
        ukbb_ancestry = ukbb_phenotypes.select(
        ...
        ukbb_ancestry = ukbb_ancestry.annotate(
            Self_reported_ancestry=hl.switch(ukbb_ancestry["f.21000.0.0"])
            .when(1, "White")
            .when(1001, "British")
            .when(1002, "Irish")
        ...
        ukbb_ancestry.write(
            get_ukbb_self_reported_ancestry_path(freeze), overwrite=args.overwrite
        )
    ...
    if args.join_ancestry:
        ...
        # pop_joint_ht.show()
        pop_joint_ht.write(
            get_joint_regeneron_ancestry_path(data_source, freeze),
            overwrite=args.overwrite,
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--join_relatedness",
        help="Join relatedness table with Regeneron relatedness inference files.",
        action="store_true",
    )
    parser.add_argument(
        "--first_degree_kin_threshold",
        help="Upper and lower kinship threshold to use for first degree relatedness comma separated",
        default="0.1767767,0.4",
    )
    parser.add_argument(
        "--second_degree_kin_threshold",
        help="Lower kinship threshold to use for second degree relatedness",
        default=0.08838835,
    )
    parser.add_argument(
        "--ibd0_parent_offspring_threshold",
        help="IBD2 cutoff to determine parent offspring vs full sibling",
        default=0.05,
    )

    parser.add_argument(
        "--filter_ukbb_ancestry",
        help="Subset UKBB phenotypes to self reported ancestry.",
        action="store_true",
    )
    parser.add_argument(
        "--join_ancestry",
        help="Join ancestry table with Regeneron and UKBB files.",
        action="store_true",
    )
