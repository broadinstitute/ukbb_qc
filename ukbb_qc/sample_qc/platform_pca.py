...
from ukbb_qc.resources.sample_qc import (
                                callrate_mt_path, platform_pca_loadings_ht_path,
                                platform_pca_results_ht_path, platform_pca_scores_ht_path
                                )
...
def main(args):
    ...
    if args.run_platform_pca:
        ...
        scores_ht.write(
            platform_pca_scores_ht_path(data_source, freeze, interval_filtered=args.apply_interval_qc_filter), 
            overwrite=args.overwrite
        )
        logger.info(f"Scores Table count: {scores_ht.count()}")
        loadings_ht.write(
            platform_pca_loadings_ht_path(data_source, freeze, interval_filtered=args.apply_interval_qc_filter), 
            overwrite=args.overwrite
        )
    ...
    if args.assign_platforms:
        ...
        platform_ht = assign_platform_from_pcs(
            scores_ht, hdbscan_min_cluster_size=args.hdbscan_min_cluster_size,
            hdbscan_min_samples=args.hdbscan_min_samples
        )
        platform_ht = platform_ht.checkpoint(
            platform_pca_results_ht_path(data_source, freeze, interval_filtered=args.apply_interval_qc_filter),
            overwrite=args.overwrite
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--run_platform_pca", help="Runs platform PCA (assumes callrate MT was computed)",
        action="store_true"
    )
    parser.add_argument(
        "--assign_platforms", help="Assigns platforms based on callrate PCA results using HDBSCAN",
        action="store_true"
    )
    parser.add_argument(
        "--hdbscan_min_samples",
    ...
    parser.add_argument(
        "--hdbscan_min_cluster_size", help="Minimum cluster size parameter for HDBSCAN.", type=int,
        default=100
    )
