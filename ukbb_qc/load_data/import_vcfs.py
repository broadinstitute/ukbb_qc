...
def main(args):
    ...
    if args.import_vqsr:
        ...
        ht = hl.split_multi_hts(ht).checkpoint(
            var_annotations_ht_path(
                data_source, freeze, "vqsr" if args.vqsr_type == "AS" else "AS_TS_vqsr"
            ),
    ...
    if args.write_raw_mt:
        ...
        drop_annots.extend(
            ["SB_TABLE", "RAW_MQandDP", "QUALapprox", "InbreedingCoeff", "END"]
        )  # 'ExcessHet'
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--import_vcfs", help="Import and write temporary raw MT", action="store_true"
    )
    parser.add_argument(
        "--import_vqsr",
        help="Import and write VQSR annotations to HT",
        action="store_true",
    )
    parser.add_argument(
        "--write_raw_mt",
        help="Trim temporary raw MT into final raw MT",
        action="store_true",
    )
    ...
    parser.add_argument(
        "-i",
        "--import_regex",
        help="Regex string containing VCF(s) for import",
        type=str,
    )
    parser.add_argument(
        "-v",
        "--import_vqsr_regex",
        help="Regex string containing VQSR VCF(s) for import",
        type=str,
    )
    parser.add_argument(
        "--import_vqsr_header",
        help="In needed can pass a specific header to use for the vcfs",
        type=str,
    )
    parser.add_argument(
        "--vqsr_type",
    ...
    parser.add_argument(
        "-n",
        "--num_partitions",
        help="Number of partitions to be used for raw MT",
        type=int,
    )
    parser.add_argument(
        "-m",
        "--num_vqsr_partitions",
        help="Number of partitions to be used for VQSR HT",
        type=int,
    )
    parser.add_argument(
        "-s",
