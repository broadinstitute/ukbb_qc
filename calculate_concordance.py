...
def write_truth_concordance(data_source: str, freeze: int, truth_sample: str, overwrite: bool) -> None:
    ...
    sites_concordance_ht.write(var_annotations_ht_path(data_source, freeze, f'{truth_sample}_concordance'), overwrite=overwrite)
    sample_concordance_ht.write(sample_annotations_table_path(data_source, freeze, f'{truth_sample}_concordance'), overwrite=overwrite)


def create_binned_concordance(data_source: str, freeze: int, truth_sample: str, metric: str, nbins: int, overwrite: bool) -> None:
    """
    ...
    if hl.hadoop_exists(binned_concordance_path(data_source, freeze, truth_sample, metric)+'/_SUCCESS') and not overwrite:
        ...
        ht = hl.read_table(var_annotations_ht_path(data_source, freeze, f'{truth_sample}_concordance'))
        ...
        high_conf_intervals = hl.import_bed(truth_samples[truth_sample]['bed'], reference_genome='GRCh38', skip_invalid_intervals=True)
        ...
        metric_ht = metric_ht.filter(~vqsr_ht[metric_ht.key].filters.contains("LowQual"))


        metric_snvs, metrics_indels, interval_metric_snvs, interval_metrics_indels = \
            metric_ht.aggregate([hl.agg.count_where(hl.is_snp(metric_ht.alleles[0], metric_ht.alleles[1])),
                                 hl.agg.count_where(~hl.is_snp(metric_ht.alleles[0], metric_ht.alleles[1])),
                                 hl.agg.count_where(hl.is_snp(metric_ht.alleles[0], metric_ht.alleles[1]) & metric_ht.interval_qc_pass),
                                 hl.agg.count_where(~hl.is_snp(metric_ht.alleles[0], metric_ht.alleles[1]) & metric_ht.interval_qc_pass)])

        snvs, indels, interval_snvs, interval_indels = ht.aggregate([hl.agg.count_where(hl.is_snp(ht.alleles[0], ht.alleles[1])),
                                     hl.agg.count_where(~hl.is_snp(ht.alleles[0], ht.alleles[1])),
                                     hl.agg.count_where(hl.is_snp(ht.alleles[0], ht.alleles[1]) & metric_ht[ht.key].interval_qc_pass),
                                     hl.agg.count_where(~hl.is_snp(ht.alleles[0], ht.alleles[1]) & metric_ht[ht.key].interval_qc_pass)
                                     ])

        ht = ht.annotate_globals(global_counts=hl.struct(snvs=metric_snvs, indels=metrics_indels, interval_snvs=interval_metric_snvs, interval_indels=interval_metrics_indels),
                                 counts=hl.struct(snvs=snvs, indels=indels, interval_snvs=interval_snvs, interval_indels=interval_indels))
        ...
        ht = add_rank(ht, -1.0*ht.score, subrank_expr={'interval_rank': ht.interval_qc_pass})
        ...
        ht = ht.annotate(rank=[
            ...
            hl.tuple(['interval_global_rank', (ht.interval_global_rank + 1) / hl.cond(
                ht.snv,
                ht.globals.global_counts.interval_snvs,
                ht.globals.global_counts.interval_indels
            )]),
            hl.tuple(['interval_truth_sample_rank', (ht.interval_rank + 1) / hl.cond(
                ht.snv,
...
def main(args):
    ...
    if args.compute_concordance:
        if args.na12878:
            ...
            write_truth_concordance(data_source, freeze, 'na12878', overwrite=args.overwrite)
    ...
    if args.bin_concordance:
        ...
        for truth_sample in truth_samples:
            for metric in metrics:
                logger.info(f"Creating binned concordance table for {truth_sample} for metric {metric}")
                create_binned_concordance(data_source, freeze, truth_sample, metric, args.n_bins, args.overwrite)
