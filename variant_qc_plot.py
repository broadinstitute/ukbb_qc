...
def get_binned_concordance_pd(data_source: str, freeze: int, truth_samples: List[str],
                              models: Union[Dict[str, str], List[str]]) -> pd.DataFrame:
    """
    ...
    def get_binned_concordance_ht(data_source: str, freeze: int, truth_samples: List[str],
                                  models: Dict[str, str]) -> hl.Table:
        """
    ...
    def compute_cumul_metrics(df: pd.DataFrame) -> pd.DataFrame:
        ...
        return df[['bin', 'min_score', 'max_score', 'n_alleles', 'tp', 'fp', 'fn', 'cum_alleles', 'cum_tp', 'cum_fp',
                   'cum_fn', 'cum_tn', 'precision', 'recall']]
...
def get_binned_concordance_chr20_pd(data_source: str, freeze: int, truth_samples: List[str],
                                    models: Union[Dict[str, str], List[str]]) -> pd.DataFrame:
    """
    Creates a pandas DF containing the binned concordance results for all given truth samples / models.
    ...
    def compute_cumul_metrics(df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['cum_alleles'] = df['n_alleles'].cumsum()
        return df[
            ['bin', 'min_score', 'max_score', 'n_alleles', 'tp', 'fp', 'cum_alleles', 'cum_tp', 'cum_fp', 'cum_fn',
             'cum_tn', 'precision', 'recall']]
...
def get_binned_models_pd(data_source: str, freeze: int, models: Union[Dict[str, str], List[str]],
                         contigs: Set[str] = None) -> pd.DataFrame:
    """
    ...
    hts = [
        aggregate_contig(hl.read_table(score_ranking_path(data_source, freeze, model_id, binned=True)), contigs)
...
def plot_training_distr(ht: hl.Table, train_id: str):
    ...
    p = hl.plot.histogram(ht.freq[0].AF, range=(0, 1), bins=100,
                          legend='gnomAD AF', title=train_id + ", n_truth_training variants = " + str(count))

    return p


def plot_score_distributions(data_source,
                             freeze,
    ...
    for model_name in model_order:
        ...
        if model_id in ['vqsr', 'cnn', 'rf_2.0.2', 'rf_2.0.2_beta']:
            ht = hl.read_table(score_ranking_path(data_source, freeze, model_id, binned=False))
        else:
            ht = hl.read_table(rf_path(data_source, freeze, 'rf_result', run_hash=model_id))
        if rank_prefix == "interval_":
            ht = ht.filter(ht.interval_qc_pass)
        ht = ht.filter(hl.is_snp(ht.alleles[0], ht.alleles[1]), keep=snv)
        binned_ht = hl.read_table(score_ranking_path(data_source, freeze, model_id, binned=True))
        binned_ht = binned_ht.filter(binned_ht.snv, keep=snv)

        if cut_is_bin:
            cut_value = binned_ht.aggregate(
                hl.agg.filter((binned_ht.bin == cut) & (binned_ht.rank_id == f'{rank_prefix}rank'), hl.agg.min(binned_ht.min_score)))
        else:
            cut_value = cut

        min_score, max_score = (-40, 200) if model_id == 'vqsr' else (0.0, 1.0)
        agg_values = ht.aggregate(hl.struct(
            score_hist=[hl.agg.hist(ht.score, min_score, max_score, 100),
                        hl.agg.filter(ht.ac_adj > 0, hl.agg.hist(ht.score, min_score, max_score, 100))],
            adj_counts=hl.agg.filter(ht.ac_adj > 0, hl.agg.counter(ht.score >= cut_value))
        ))
        score_hist = agg_values.score_hist
        adj_cut = '{0:.2f}'.format(
            100 * agg_values.adj_counts[True] / (agg_values.adj_counts[True] + agg_values.adj_counts[False]))
        ...
        for adj in [False, True]:
            ...
            p_cumul = plot_hail_hist_cumulative(score_hist[adj], title=title + ', cumulative',
...
def preprocess_sib_singleton_ht(ht, variables):
    ...
    total_count = total_count.annotate(order=order.index(total_count.dataset)).key_by(*keys)
    totals_ht = (total_count.group_by(*keys).aggregate(bin_total=hl.agg.sum(total_count['n_group']))).key_by(*keys)
    total_count = total_count.annotate(fraction=(total_count.n_group / totals_ht[total_count.key].bin_total)*100)
    total_count = total_count.filter(total_count.pass_status).to_pandas().sort_values(by=['order'])
    return total_count


def sib_singleton_bar_plot(source_data,
                           x_group,
                           n_group,
                           x_order,
                           title="",
    ...
    p = figure(x_range=x_order, plot_height=plot_height, plot_width=plot_width, title=title,
               tools=[hover, 'save', 'pan', 'box_zoom', 'reset', 'wheel_zoom'])
    p.vbar(x=x_group, top=n_group, width=0.9, source=source_data, line_color='white',
...
def plot_metric(df: pd.DataFrame,
    ...
    def get_row(df: pd.DataFrame, y_name: str, cols: List[str], y_fun: Callable[[pd.Series], Union[float, int]],
                titles: List[str], link_cumul_y: bool, cut: int = None, extra_lines=None,
                expectation_line=None) -> Row:
        """
        ...
        def get_plot(data_source: ColumnDataSource, y_name: str, y_col_name: str, titles: List[str],
                     data_ranges: Tuple[DataRange1d, DataRange1d], cut: int = None, extra_lines=None,
                     expectation_line=None) -> Plot:
            """
            ...
            if extra_lines:
                for line in extra_lines:
                    p.add_layout(Span(location=line, dimension='width', line_color='red', line_dash='dashed'))
            if expectation_line:
                p.add_layout(Span(location=expectation_line, dimension='width', line_color='green', line_dash='dashed',
                                  line_width=3))
            ...
            for model in set(data_source.data['model']):
                view = CDSView(source=data_source,
                               filters=[BooleanFilter([x == model for x in data_source.data['model']])])
                circles.append(
            ...
            legend1 = Legend(items=circles[:4], orientation='horizontal', location=(0, 0), click_policy="hide")
            legend2 = Legend(items=circles[4:], orientation='horizontal', location=(0, 0), click_policy="hide")
        ...
        if no_cuml:
            ...
            return Row(get_plot(data_source, y_name, 'non_cumul', titles, non_cumul_data_ranges, cut),
                   get_plot(data_source, y_name, 'cumul', [titles[0] + ', cumulative'] + titles[1:], cumul_data_ranges,
                            cut, extra_lines, expectation_line))
    ...
    if plot_all:
        ...
        for adj in adj_strats:
            ...
            plot_df = prepare_pd(df.loc[df.rank_id == f'{rank_prefix}{adj}rank'], cols, colors, size_prop)
            if len(plot_df) > 0:
                children.append(
                    get_row(plot_df, y_name, cols, y_fun, titles, link_cumul_y, cut, extra_lines, expectation_line))
            else:
    ...
    if plot_bi_allelics:
        ...
        for adj in adj_strats:
            for biallelic_rank in ['', 'biallelic_']:
                titles = [y_name, 'Bi-allelic variants ({} rank)'.format(
                    'overall' if not adj and not biallelic_rank else ' '.join(
                        [adj[:-1], biallelic_rank[:-1]]).lstrip())]
                plot_df = prepare_pd(df.loc[df.bi_allelic & (df.rank_id == f'{rank_prefix}{adj}{biallelic_rank}rank')], cols, colors,
                                     size_prop)
                if len(plot_df) > 0:
    ...
    if plot_singletons:
        ...
        for adj in adj_strats:
            for singleton_rank in ['', 'singleton_']:
                titles = [y_name, 'Singletons ({} rank)'.format(
                    'overall' if not adj and not singleton_rank else " ".join(
                        [adj[:-1], singleton_rank[:-1]]).lstrip())]
                plot_df = prepare_pd(df.loc[df.singleton & (df.rank_id == f'{rank_prefix}{adj}{singleton_rank}rank')], cols, colors,
                                     size_prop)
                if len(plot_df) > 0:
                    children.append(get_row(plot_df, y_name, cols, y_fun, titles, link_cumul_y, cut))
                else:
    ...
    if plot_bi_allelic_singletons:
        ...
        for adj in adj_strats:
            for bisingleton_rank in ['', 'biallelic_singleton_']:
                titles = [y_name, 'Bi-allelic singletons ({} rank)'.format(
                    'overall' if not adj and not bisingleton_rank else " ".join(
                        [adj[:-1], bisingleton_rank[:-1].replace("_", " ")]).lstrip())]
                plot_df = prepare_pd(
                    df.loc[df.bi_allelic & df.singleton & (df.rank_id == f'{rank_prefix}{adj}{bisingleton_rank}rank')], cols, colors,
                    size_prop)
                if len(plot_df) > 0:
                    children.append(get_row(plot_df, y_name, cols, y_fun, titles, link_cumul_y, cut))
                else:
...
def plot_pr_curve(truth_sample: str, pr_df: pd.DataFrame, hover, rank: str, snv: bool, y_range,
                  size_prop: str = None,
                  bins_to_label: List[int] = None, colors: Dict[str, str] = None, model_order=None,
                  extra_point=None):
    if extra_point:
         extra_point_df = pd.DataFrame(data={'recall': [extra_point[0]], 'precision': [extra_point[1]]})
    vartype = " SNPs" if snv else " Indels"
    p = figure(title=truth_sample[0].upper() + truth_sample[1:] + vartype,
               x_axis_label='Recall',
    ...
    if extra_point:
        ...
        circles.append(("Regeneron GL", [p.triangle('recall',
                                                'precision',
                                                size=15,
                                                color=colors["Regeneron GL"], source=source)]))
    legend1 = Legend(items=circles[:3], orientation='horizontal', location=(0, 0), click_policy="hide")
    legend2 = Legend(items=circles[3:], orientation='horizontal', location=(0, 0), click_policy="hide")
    p.add_layout(legend2, 'above')
    p.add_layout(legend1, 'above')
    set_plots_defaults(p)
    p.legend.label_text_font_size = f'11pt'
...
def plot_concordance_pr(
    ...
    for rank in ranks:
        ...
        for truth_sample in set([g[1] for g in pr_df.groups]):
            p_snv = plot_pr_curve(truth_sample, pr_df, hover, rank, True, y_range_snv, size_prop, bins_to_label[0],
                                  colors, model_order=model_order, extra_point=extra_point_snv)
            p_indel = plot_pr_curve(truth_sample, pr_df, hover, rank, False, y_range_indel, size_prop, bins_to_label[1],
                                    colors, model_order=model_order, extra_point=extra_point_indel)
